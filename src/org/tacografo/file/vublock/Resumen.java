...
public class Resumen extends Block{
	...
	private OctetString memberStateCertificate;
	
	// 2.121. VuCertificate
	// Certificado de la clave p�blica de una unidad intravehicular.
	// VuCertificate::= Certificate
	
	private OctetString VUcertificate;
	...
	public Resumen(byte[] bytes) throws Exception{
		...
		Certificate c=new Certificate(Arrays.copyOfRange(bytes, start, start+=Sizes.MEMBERSTATECERTIFICATE.getSize()));				
		this.memberStateCertificate=c.getCertificate();
		
		c=new Certificate(Arrays.copyOfRange(bytes, start, start+=Sizes.MEMBERSTATECERTIFICATE.getSize()));
		this.VUcertificate =c.getCertificate();
		
		IA5String ia5=new IA5String(Arrays.copyOfRange(bytes, start, start+=Sizes.VEHICLEIDENTIFICATIONNUMBER.getSize()));
		this.vehicleIdentificationNumber= ia5.getiA5String();
		
		this.vehicleRegistrationIdentification=new VehicleRegistrationIdentification(Arrays.copyOfRange(bytes, start, start+=Sizes.VEHICLEREGISTRATIONIDENTIFICATION_TREP1.getSize()));
		
		this.currentDateTime= RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=Sizes.CURRENTDATETIME.getSize()));
		
		this.minDownloableTime=RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=Sizes.MINDOWNLOADLETIME.getSize()));
		
		this.maxDonloadbleTime=RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=Sizes.MAXDOWNLOADLETIME.getSize()));
		//tengo que mirar los bytes
		this.cardSlotsStatus=Arrays.copyOfRange(bytes, start, start+=Sizes.CARDSLOTSSTATUS.getSize());
		
		this.downloadingTime=RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=Sizes.DOWNLOADINGTIME.getSize()));
		this.fullcardNumber=new FullCardNumber(Arrays.copyOfRange(bytes, start, start+=Sizes.FULLCARDNUMBER.getSize()));
		
		this.companyOrWorkshopName= new Name(Arrays.copyOfRange(bytes, start, start+=Sizes.COMPANYORWORKSHOPNAME.getSize()));
		this.noOfLocks= Number.getShort_8(Arrays.copyOfRange(bytes, start, start+=Sizes.NOOFLOCKS.getSize()));
		if(this.noOfLocks>0){
			this.vuCompanyLocksRecords=new ArrayList<VuCompanyLocksRecord>();
			this.getListVuCompanyLocksRecords(Arrays.copyOfRange(bytes, start, start+=Sizes.VUCOMPANYLOCKSRECORD.getSize()*this.noOfLocks));
		}else{
	 ...
	 */
	public OctetString getMemberStateCertificate() {
	 ...
	 */
	public void setMemberStateCertificate(OctetString memberStateCertificate) {
	 ...
	 */
	public OctetString getVUcertificate() {
	 ...
	 */
	public void setVUcertificate(OctetString vUcertificate) {
	...
	public String toString() {
		return "Resumen [memberStateCertificate=" + memberStateCertificate.getOctetString() + "\n, VUcertificate=" + VUcertificate.getOctetString()
