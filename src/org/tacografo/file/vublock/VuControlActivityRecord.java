...
public class VuControlActivityRecord {
	...
	public VuControlActivityRecord(byte[] bytes) throws UnsupportedEncodingException{
		int start=0;
		this.controlType=new ControlType(Arrays.copyOfRange(bytes, start, start+=Sizes.CONTROLTYPE.getSize()));
		this.controlTime= RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=Sizes.CONTROLTIME.getSize()));
		this.controlCardNumber= new FullCardNumber(Arrays.copyOfRange(bytes, start, start+=Sizes.FULLCARDNUMBER.getSize()));
		this.downloadPeriodBeginTime=RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=Sizes.DOWNLOADPERIODBEGINTIME.getSize()));
		this.downloadPeriodEndTime=RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=Sizes.DOWNLOADPERIODENDTIME.getSize()));
	}
