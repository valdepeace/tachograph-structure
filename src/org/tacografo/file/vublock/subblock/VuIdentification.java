...
public class VuIdentification {
	...
	public VuIdentification(byte[] bytes) throws IOException {
		int start=0;
		Name n=new Name(Arrays.copyOfRange(bytes, start, start+=Sizes.VUMANUFACTURERNAME.getSize()));
		this.vuManufacturerName=n.getName();
		n=new Name(Arrays.copyOfRange(bytes, start, start+=Sizes.VUMANUFACTURERADDRESS.getSize()));
		this.VuManufacturerAddress=n.getName();
		IA5String ia5s=new IA5String(Arrays.copyOfRange(bytes, start, start+=Sizes.VUPARTNUMBER.getSize()));
		this.VuPartNumber=ia5s.getiA5String();
		this.VuSerialNumber=new ExtendedSerialNumber(Arrays.copyOfRange(bytes, start, start+=Sizes.VUSERIALNUMBER.getSize()));
		this.VuSoftwareIdentification=new VuSoftwareIdentification(Arrays.copyOfRange(bytes, start, start+=Sizes.VUSOFTWAREIDENTIFICATION.getSize()));
